...
const books = livros[0].books

const getInfos = (keyword = '') =>{
    let erro = true;
    let infos = [];

    if(!keyword == ''){
        books.forEach(getGenero => {
            if(getGenero.title.toLowerCase().includes(keyword.toLowerCase()) || getGenero.subtitle.toLowerCase().includes(keyword.toLowerCase()))
            infos.push(getGenero);
            erro = false;
        })
    }
    else
